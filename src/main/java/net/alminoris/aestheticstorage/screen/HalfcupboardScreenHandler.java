...
public class HalfcupboardScreenHandler extends ScreenHandler
{
    ...
    //Client
    public HalfcupboardScreenHandler(int syncId, PlayerInventory inventory, BlockPosPayload payload)
    {
        this(syncId, inventory, (HalfcupboardBlockEntity) inventory.player.getWorld().getBlockEntity(payload.pos()));
    }
                                     ...
                                     HalfcupboardBlockEntity blockEntity)
    {
        ...
        for(int i = 1; i < 3; i++)
        {
            for(int j = 0; j < 2; j++)
            {
                this.addSlot(new Slot(INVENTORY, l++, 53 + i * 18, 27 + j * 18));
            }
        }
